...
static PROPERTIES: OnceBox<[Option<Properties>; 256]> = OnceBox::new();

pub fn get_properties() -> &'static [Option<Properties>; 256] {
...
pub type GasCosts = [Option<u16>; 256];

static FRONTIER_GAS_COSTS: OnceBox<GasCosts> = OnceBox::new();
static HOMESTEAD_GAS_COSTS: OnceBox<GasCosts> = OnceBox::new();
static TANGERINE_GAS_COSTS: OnceBox<GasCosts> = OnceBox::new();
static SPURIOUS_GAS_COSTS: OnceBox<GasCosts> = OnceBox::new();
static BYZANTIUM_GAS_COSTS: OnceBox<GasCosts> = OnceBox::new();
static CONSTANTINOPLE_GAS_COSTS: OnceBox<GasCosts> = OnceBox::new();
static PETERSBURG_GAS_COSTS: OnceBox<GasCosts> = OnceBox::new();
static ISTANBUL_GAS_COSTS: OnceBox<GasCosts> = OnceBox::new();
static BERLIN_GAS_COSTS: OnceBox<GasCosts> = OnceBox::new();
static LONDON_GAS_COSTS: OnceBox<GasCosts> = OnceBox::new();
static SHANGHAI_GAS_COSTS: OnceBox<GasCosts> = OnceBox::new();

#[allow(clippy::needless_range_loop)]
fn frontier_gas_costs() -> &'static GasCosts {
    FRONTIER_GAS_COSTS.get_or_init(|| {
        let mut table = Box::new([None; 256]);
...
fn homestead_gas_costs() -> &'static GasCosts {
    HOMESTEAD_GAS_COSTS.get_or_init(|| {
        let mut table = Box::new(*frontier_gas_costs());
        table[OpCode::DELEGATECALL.to_usize()] = Some(40);
        table
    })
}

fn tangerine_gas_costs() -> &'static GasCosts {
    TANGERINE_GAS_COSTS.get_or_init(|| {
        let mut table = Box::new(*homestead_gas_costs());
...
fn spurious_gas_costs() -> &'static GasCosts {
    SPURIOUS_GAS_COSTS.get_or_init(|| Box::new(*tangerine_gas_costs()))
}

fn byzantium_gas_costs() -> &'static GasCosts {
    BYZANTIUM_GAS_COSTS.get_or_init(|| {
        let mut table = Box::new(*spurious_gas_costs());
...
fn constantinople_gas_costs() -> &'static GasCosts {
    CONSTANTINOPLE_GAS_COSTS.get_or_init(|| {
        let mut table = Box::new(*byzantium_gas_costs());
...
fn petersburg_gas_costs() -> &'static GasCosts {
    PETERSBURG_GAS_COSTS.get_or_init(|| Box::new(*constantinople_gas_costs()))
}

fn istanbul_gas_costs() -> &'static GasCosts {
    ISTANBUL_GAS_COSTS.get_or_init(|| {
        let mut table = Box::new(*petersburg_gas_costs());
...
fn berlin_gas_costs() -> &'static GasCosts {
    BERLIN_GAS_COSTS.get_or_init(|| {
        let mut table = Box::new(*istanbul_gas_costs());
...
fn london_gas_costs() -> &'static GasCosts {
    LONDON_GAS_COSTS.get_or_init(|| {
        let mut table = Box::new(*berlin_gas_costs());
        table[OpCode::BASEFEE.to_usize()] = Some(2);
        table
    })
}

fn shanghai_gas_costs() -> &'static GasCosts {
    SHANGHAI_GAS_COSTS.get_or_init(|| Box::new(*london_gas_costs()))
}

pub fn gas_costs(revision: Revision) -> &'static GasCosts {
    match revision {
