...
fn synthetic_benchmarks(c: &mut Criterion) {
    ...
    c.bench_function("/total/synth/loop_v1", |b| {
        b.to_async(&r)
            .iter(|| bench_execute(generate_loop_v1(Bytecode::new())))
    });
    c.bench_function("/total/synth/loop_v2", |b| {
        b.to_async(&r)
            .iter(|| bench_execute(generate_loop_v2(Bytecode::new())))
    });
    ...
    for params in params_list {
        c.bench_function(
            ...
            |b| {
                b.to_async(&r)
                    .iter(|| bench_execute(generate_code(&params)))
            },
        );
    }
}

async fn bench_execute(code: Bytecode) {
    AnalyzedCode::analyze(code.build())
        .execute(
            &mut MockedHost::default(),
            &mut NoopTracer,
            Message {
                kind: CallKind::Call,
                gas: i64::MAX,
