...
use evmodin::{
    ...
    AnalyzedCode, CallKind, Message, OpCode, Revision,
};
use hex_literal::hex;
...
impl Display for CodeParams {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "{}/{}{}",
            PROPERTIES[self.opcode.to_usize()].unwrap().name,
            self.opcode,
            self.mode as u8
        )
    }
...
criterion_group!(
    ...
    targets = synthetic_benchmarks
);

// async fn run_loop_contract() {}
...
//         b.iter(|| r.block_on(run_loop_contract()))
//     });
// }
